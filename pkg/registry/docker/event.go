...
func transformEvent(event *APIEvents) {
	...
	if event.Action == "" && event.Type == "" {
		...
		default:
			...
			if event.From != "" {
				event.Actor.Attributes["registry"] = event.From
			}
		}
	} else {
		if event.Status == "" {
			if event.Type == "registry" || event.Type == "container" {
				event.Status = event.Action
			} else {
				// Because just the Status has been overloaded with different Types
				// if an event is not for an registry or a container, we prepend the type
		...
		if event.From == "" {
			event.From = event.Actor.Attributes["registry"]
		}
	}
}
